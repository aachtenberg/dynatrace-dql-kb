...
# Set DT_REDACT_TENANT=1 to write a placeholder host into the generated docs
# instead of the real tenant URL. Use this when committing the generated docs to
# a shared repo so you don't publish a tenant identifier. Local-only generation
# can leave it unset to keep the real URL for provenance.
DT_REDACT_TENANT = os.getenv("DT_REDACT_TENANT", "").lower() in ("1", "true", "yes")
REDACTED_URL = "https://<your-env>.apps.dynatrace.com"
...
def _source_url() -> str:
    """URL to stamp into generated docs — redacted when DT_REDACT_TENANT is set."""
    return REDACTED_URL if DT_REDACT_TENANT else DT_ENVIRONMENT_URL
...
def populate_metric_keys():
    ...
    lines = [
        ...
        f"# Auto-generated by dt_fetch.py from {_source_url()}",
